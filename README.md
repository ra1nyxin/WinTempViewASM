...
- 支持目标扫描、空间预估和真实清理。
- 扫描结果统计文件数、目录数、跳过数，以及预计可释放空间；完整结果以启动日志为准，界面结果区从日志读取内容，日志较小时显示最新完整结果，日志较大时显示尾部预览。
- 预计可释放空间使用 `MB / GB` 双格式显示，并保留两位小数。
- 清理任务运行在后台线程，进度通过窗口消息回传 UI 线程，避免界面卡死。
- 清理进度显示已处理项数和剩余项数，不显示预计时间。
- 启动时在程序所在目录生成 `winTempView-log-YYYY-MMDD_HHMMSS.txt` 实时日志，便于排查启动、建窗、扫描和清理流程；扫描、预估和清理完成后也会把完整结果写入日志作为显示兜底。
- 可执行文件内嵌 `requireAdministrator` manifest，启动时默认请求管理员权限。
- 支持启动参数 `--clean-all`，用于跳过 GUI 直接清理全部大分类，并把结果写入启动日志。
...
- 结果显示容错：扫描、预估和清理的完整结果以启动日志为权威记录；结果区使用只读多行文本框，从日志文件读取最新结果。日志较小时从最近的 `[result] begin` 开始显示完整结果，日志较大时显示最后约 300 行或约 180KB 的结果尾部，扫描完成后自动滚动到底部，避免大量规则输出直接压垮 UI 控件。
- 日志容错：启动日志实时写入并刷新到磁盘，用于定位启动、建窗、扫描、清理和 worker 收尾流程中的问题；每次扫描、预估和清理生成的完整结果会写入 `[result] begin` 与 `[result] end` 区间，便于在 UI 显示异常时回看。
...
- 办公、协作和效率软件：WPS Office、WPS Cloud、金山 PDF、腾讯文档、钉钉、飞书、Lark、Slack、Teams、Teams classic、Zoom、Skype、LINE、Viber、Element、Mattermost、Notion、Figma、Obsidian、Canva、Typora、XMind、蓝湖、Pixso、MasterGo、Axure RP、墨刀/MockingBot、Wondershare Mockitt、亿图图示、MindMaster、幕布、Joplin、Zotero、Evernote/印象笔记、Thunderbird、Mailspring、Todoist、Trello、Miro、Linear、网易邮箱大师、有道词典、有道云笔记、有道翻译、滴答清单、百度 Hi、千牛、语雀、PowerToys、OneDrive、Office、Outlook、Everything、Motrix、uTools、Snipaste、PicGo。覆盖应用缓存、WebView 缓存、Code Cache、GPUCache、Crashpad、日志、临时目录、邮件客户端缓存、Office 文档缓存、腾讯文档 Code Cache/GPUCache、有道词典缓存和原型设计工具缓存。额外覆盖 D/E 盘常见安装目录中的 WPS Office、DingTalk、Feishu、TencentDocs 缓存和日志。
...
- AI、开发和 API 工具：VS Code、VS Code Insiders、VSCodium、Cursor、Windsurf、Trae、Visual Studio、Visual Studio Installer、VSCommon、MSBuild、VSTest、JetBrains、本地 JetBrains Toolbox、Android Studio、Eclipse、NetBeans、GitHub Desktop、SourceTree、TortoiseGit、Cmder、Postman、Apifox、ApiPost、Insomnia、GitKraken、DBeaver、Navicat、MongoDB Compass、RedisInsight、DbGate、Beekeeper Studio、Fiddler Everywhere、Charles、Wireshark、Burp Suite、Nmap、Claude、ChatGPT、Perplexity、LM Studio、Ollama。覆盖 CachedData、Cache、Code Cache、GPUCache、Crashpad、日志、Service Worker CacheStorage、DawnCache、blob_storage、扩展 VSIX 缓存、ComponentModelCache、工具箱缓存、Android Studio 缓存、Eclipse p2 cache、NetBeans cache/logs、数据库客户端缓存、抓包调试工具缓存和本地开发工具缓存。额外覆盖 D/E 盘常见便携目录中的 VSCode data user-data Cache、JetBrains system caches、Postman Cache、Apifox Cache。
- 开发语言、包管理器、云工具和构建缓存：npm、npm local cache、npm user cache、npm `_cacache`、npm `_logs`、npx cache、Corepack、Volta、pip、pip user cache、pip-tools、uv、pipx、PDM、Poetry、Rye、NuGet packages、NuGet HTTP cache、NuGet plugins-cache、NuGetScratch、Gradle、Gradle wrapper/daemon/native/buildOutputCleanup、Gradle modules-2、transforms-3、transforms-4、jars-9、kotlin-dsl、build-cache-1、journal-1、workers、`.gradle\\.tmp`、Fabric Loom、ForgeGradle、NeoForge Gradle、Quilt Loom、Architectury Loom、Paperweight、MinecraftDev、MCreator、Maven、本地 Maven wrapper 和 wrapper dists、Ivy2、sbt、Coursier、Kotlin cache、Kotlin Native konan cache、Sonar cache、Yarn、Yarn Berry、Yarn user cache、Yarn v6 cache、pnpm、pnpm user store、pnpm cache、Cargo registry/git、rustup downloads/tmp、Poetry、Conda、Conda cache、Anaconda pkgs、Miniconda pkgs、Julia compiled/logs/scratchspaces、RStudio cache/logs、Hugging Face、Hugging Face hub/datasets、ModelScope、Android SDK temp、Android Emulator temp、Android build/Gradle cache、Android Studio system cache、Genymotion logs、Flutter pub cache、Flutter bin cache、FVM、Go build cache、Go module cache、Docker Desktop、Docker Desktop logs、Docker buildx、Rancher Desktop logs、Podman cache、Kubernetes client cache、Helm cache、minikube cache、Terraform plugin cache、Bazel、node-gyp、Electron、Playwright、Selenium、Cypress、Turbo/Turborepo、Vite、Expo、Prisma、PyTorch、PyTorch extensions、matplotlib、Jupyter runtime、pre-commit、vcpkg downloads、Conan、Composer、RubyGems、Dart Pub、.NET HTTP cache、Bun、Deno、Ruff、mypy、ccache、sccache、Ninja build logs、Emscripten cache、Jenkins cache/war、GitLab Runner cache、Scoop cache、Chocolatey cache、winget diagnostic cache、Azure CLI logs/command cache、AWS CLI cache、gcloud logs/cache、Alibaba Cloud CLI cache/logs、Tencent Cloud CLI cache/logs、Huawei Cloud CLI cache/logs。额外覆盖 C/D/E/F/G/H/I/J 盘常见开发目录中的 `dev\\node_modules\\.cache`、`projects\\node_modules\\.cache`、`workspace\\node_modules\\.cache`，以及 D/E/F/G/H/I/J 盘常见 Gradle 用户缓存、nodejs cache、node_modules/.cache、Android .gradle 和 Android build-cache。
- 国产开发工具链：微信开发者工具、HBuilderX、DevEco Studio、HarmonyOS SDK、Taro、uni-app、Tencent CloudBase CLI。覆盖 Cache、Code Cache、GPUCache、Crashpad、日志、本地构建缓存、DevEco system caches、Huawei SDK `.temp`、HBuilderX temp、uni-app CLI cache 和 CloudBase cache。
...
- 游戏和图形开发工具：Unity cache、Unity shader cache、Unity Asset Store cache、Unity Hub Cache/logs、Unreal DerivedDataCache、Unreal logs、Epic Unreal DDC、Godot editor cache/logs、Cocos Dashboard、Cocos Creator、LayaAir IDE、Egret Launcher。覆盖可再生成缓存、shader cache、Code Cache、GPUCache、日志和派生数据缓存。额外覆盖 D/E/F/G/H/I/J 盘常见 Unity cache/ShaderCache 和 UnrealEngine DerivedDataCache/Logs 目录。
...
普通启动会打开 GUI，清理范围由界面中的大分类勾选状态决定。

```powershell
.\winTempView.exe
```

命令行参数 `--clean-all` 会跳过 GUI，直接按所有大分类已选中的状态执行真实清理，并在程序所在目录生成启动日志和清理结果日志。该模式主要用于自动化验证、虚拟机测试和需要明确一键全量清理的场景。

```powershell
.\winTempView.exe --clean-all
```

`--clean-all` 不弹出清理确认窗口；调用该参数即表示确认清理所有内置大分类。清理逻辑仍然使用固定规则表、默认清理开关、路径安全校验、reparse point 跳过、根目录保护和删除统计。
...
- workflow 在 `windows-2022` 上安装或启用 MinGW-w64 GCC。
- 使用 `windres` 编译资源文件。
- 使用静态优化 release 命令编译 `winTempView.exe`。
- 运行 `winTempView.exe --clean-all` 做命令行清理路径 smoke test，并检查日志结果标记。
